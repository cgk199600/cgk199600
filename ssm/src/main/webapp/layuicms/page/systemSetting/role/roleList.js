layui.use(['form','layer','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        elem: '#roleList',//通过ID获取到页面表格
        url : '/ssm/role/getTableList.do',//通过json获取数据
        ...
        cols : [[
            {field: 'rolename', title: '标题', minWidth:100, align:"center"},
    ...
    function updateObj(edit){//主页面
        var index = layui.layer.open({//这里用layui.layer是为了防止layer可能用父级layer确保本页面的layer
            title : "更改用户",
            type : 2,
            content : "roleUpdate.html?id="+edit.id,//页面开启时传入ID
            success : function(layero, index){
