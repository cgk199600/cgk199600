...
<input type="hidden" name="id" id="id" value="{{ d.id }}"/>
   <div class="layui-form-item">
    ...
    <div class="layui-input-block">
      <input type="text" name="name" value="{{ d.name }}" required  lay-verify="required" placeholder="请输入姓名" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-form-item">
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-block"><!--required|phone|number验证电话  -->
      <input type="text" name="phone" value="{{ d.phone }}" required  lay-verify="required|phone|number" placeholder="请输入手机号码" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-form-item">
    <label class="layui-form-label">邮箱</label>
    <div class="layui-input-block"><!--验证email  -->
      <input type="text" name="email" value="{{ d.email }}" required  lay-verify="required|email" placeholder="请输入邮箱" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-form-item">
		...
		var id=location.href.split("=")[1];//切割域名到最后的ID
		 $.ajax({
			  ...
			  success:function(data){//成功请求到后端，后端返回给前端内容data
				  var getTpl = demo.innerHTML//取到数据
				  var view = document.getElementById('view');//取到ID
				  laytpl(getTpl).render(data.data, function(html){//进行渲染layui写死的变量名是d代表数据
				    view.innerHTML = html;
				  })
			  form.render();
			  }
